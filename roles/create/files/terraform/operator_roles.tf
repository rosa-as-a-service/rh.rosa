...
data "aws_iam_policy_document" "operator_role_trust_policy" {
  ...
  statement {
    ...
    principals {
      type        = "Federated"
      identifiers = ["arn:${data.aws_partition.current.partition}:iam::${data.aws_caller_identity.current.account_id}:oidc-provider/${rhcs_rosa_oidc_config.oidc_config.oidc_endpoint_url}"]
    }
    condition {
      test     = "StringEquals"
...
resource "aws_iam_role" "operator_role" {
  count                = length(local.operator_roles_properties)
  name                 = substr("${var.operator_role_prefix}-${local.operator_roles_properties[count.index].operator_namespace}-${local.operator_roles_properties[count.index].operator_name}", 0, 64)
  permissions_boundary = var.permissions_boundary != "" ? var.permissions_boundary : null
  path                 = local.path
  assume_role_policy   = data.aws_iam_policy_document.operator_role_trust_policy[count.index].json
...
resource "time_sleep" "operator_roles_wait" {
  create_duration = "20s"
  triggers = {
