...
resource "aws_iam_role" "account_role" {
  count                = length(local.account_roles_properties)
  name                 = substr("${var.account_role_prefix}-${local.account_roles_properties[count.index].role_name}-Role", 0, 64)
  permissions_boundary = var.permissions_boundary != "" ? var.permissions_boundary : null
  path                 = local.path
  assume_role_policy   = data.aws_iam_policy_document.account_role_trust_policy[count.index].json
