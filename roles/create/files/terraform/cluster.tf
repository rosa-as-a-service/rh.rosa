...
resource "rhcs_cluster_rosa_classic" "rosa_sts_cluster" {
  ...
  private                     = var.private
  aws_subnet_ids              = length(data.aws_subnet.tenent_subnet_id) >= 3 ? [for subnet_id in data.aws_subnet.tenent_subnet_id : subnet_id.id] : lookup(data.aws_subnet.tenent_subnet_id[0], "id")
  availability_zones          = length(data.aws_subnet.tenent_subnet_id) >= 3 ? [for subnet_id in data.aws_subnet.tenent_subnet_id : subnet_id.availability_zone] : lookup(data.aws_subnet.tenent_subnet_id[0], "availability_zone")
